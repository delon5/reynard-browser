    ...
    sessionConnected:
        ...
        if (!vAttachReplyIsStopPacket(normalisedAttachReply)) {
            if (vAttachReplyIsRefusal(normalisedAttachReply)) {
                ...
                // ESRCH, not MakeError(AttachDebugProxyFailed). A refusal
                // is about THIS target - it exited between spawn and
                // attach, or something else already holds it - and -12
                // would reach handleJITFailure, which latches JIT off for
                // every later child and raises the full-screen failure. A
                // tab closed while its own attach was in flight must not
                // cost the whole launch its JIT. attachToProcess treats
                // ESRCH as a per-child failure: this child is told FALSE,
                // nothing is latched.
                if (error) *error = [NSError errorWithDomain:NSPOSIXErrorDomain
                                                        code:ESRCH
                                                    userInfo:@{NSLocalizedDescriptionKey: @"vAttach was refused for this process"}];
                freeDebugSession(&session);
                return NO;
            }
