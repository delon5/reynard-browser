            ...
            for pid in stillAlive {
                ...
                self.attachWorkQueue.async {
                    ...
                    guard Self.isApplicationActiveFromAnyQueue else {
                        ...
                        //
                        // No ReportJITStatusForChild: burning the
                        // single-use pipe with a FALSE would foreclose the
                        // TRUE the re-drained attach below can still send.
                        self.cancelPreflightWatchdog(for: pid)
                        // Handed back to the deferred drain, exactly as the pre-slot
                        // inactive branch does - see the review amendment to
                        // fix_attach_gates_and_helper_guards.py. markAttached already
                        // ran and the drain skips attached pids, so it is un-marked
                        // first; otherwise this pid had no route back to JIT until
                        // the next background/foreground cycle, and Control Centre
                        // during a page load cost those tabs their JIT.
        ...
        attachQueue.async {
            ...
            self.attachWorkQueue.async {
                ...
                guard Self.isApplicationActiveFromAnyQueue else {
                    ...
                    //
                    // No ReportJITStatusForChild. The deferral this
                    // mirrors, a few lines up, is silent for the same
                    // reason: burning the single-use report pipe with a
                    // FALSE would foreclose the real TRUE the re-drained
                    // attach below can still deliver.
                    self.cancelPreflightWatchdog(for: pid)
                    // Handed back to the deferred drain, exactly as the pre-slot
                    // inactive branch does - see the review amendment to
                    // fix_attach_gates_and_helper_guards.py. markAttached already
